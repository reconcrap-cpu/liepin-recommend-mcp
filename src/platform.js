...
export function buildSkillExportPayload({
  ...
  return {
    ...
    chat: {
      ...
      criteriaOmittedMeansCollectCv: true,
...
function buildSkillExportMarkdown(payload = {}) {
  return [
    ...
    "- 启动聊天页任务前先调用 `liepin_chat_options`，只询问岗位、是否只扫未读（`unread_only`）、目标人数，以及用户是否要提供 AI 筛选标准；`criteria` 留空时进入 collect-CV 模式，不调用 LLM，只检查/索要尚未请求的简历；聊天任务默认使用 Boss chat aggressive/high 休息策略（`human_behavior.restLevel=\"high\"`，collect-CV 每候选人后休息 5-8 秒）；目标人数支持正整数，也支持 `all`、`全部`、`所有`、`扫到底`、`扫完所有人选` 等全量扫描表达，含义是扫到聊天列表底部/平台上限，不要再要求用户改成整数；参数确认后只能调用 `liepin_chat_start`，严禁调用 `liepin_recommend_start` 或 `liepin_recommend_chat_start`。",
    ...
    "- 长跑鲁棒性默认启用 `robustness_mode=recover`；只有需要复现旧行为时才显式传 `robustness_mode=off`，需要只记录不恢复时传 `robustness_mode=observe`。",
