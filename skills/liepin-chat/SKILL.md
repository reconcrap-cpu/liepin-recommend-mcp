...
- 启动前检查：`liepin_doctor`（必须传 `require_chat_page=true`；必要时 `provider_check=true`）
...
- 每次新的 chat start 前必须先通过 `liepin_doctor(require_chat_page=true)`，且 `screening-config.json` 的 `baseUrl/apiKey/model` 不能是占位值。
